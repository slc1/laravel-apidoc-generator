...
```bash
curl -X {{$parsedRoute['methods'][0]}} "{{config('app.docs_url') ?: config('app.url')}}/{{ltrim($parsedRoute['uri'], '/')}}" \
...
var settings = {
    "async": true,
    "crossDomain": true,
    "url": "{{config('app.docs_url') ?: config('app.url')}}/{{ltrim($parsedRoute['uri'], '/')}}",
...
@if(isset($parsedRoute['response']) && $parsedRoute['showresponse'])
> Example response:
